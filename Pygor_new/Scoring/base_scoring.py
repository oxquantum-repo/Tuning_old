...
def peak_count_score(trace,x_vals,height=0.0178, prominence=0.107,offset=-2e-10,maxval=5e-10):
...
def dot_periodicity_score(traces,x_vals,height=0.0178, prominence=0.107,weight=0.1,default=0.2,func=np.average,**kwargs):
#
#STILL NEEDS WORk
#
#
    ...
    return 1-np.tanh(func(scores)*weight)
    
    
    
    
    
def dot_spacing_score(traces,x_vals,height=0.0178, prominence=0.107,func=np.average,**kwargs):
    ...
    for i in range(i_iter[0].size):
        ...
        if len(comp[0])<2 or len(comp[1])<2:
            ...
            dist = np.linalg.norm(diffs[0]-diffs[1])/np.sqrt(diffs[0].size)
