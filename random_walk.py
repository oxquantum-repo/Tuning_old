...
class MH_MCMC_Hypersurface(multiprocessing.Process):
    ...
    def reset(self, z=None, L_current=None, max_steps=1000, stopper=None, result_sender=None):
        ...
        self.counter = 0
        if self.history:
            self.history_all = deque(maxlen=10000)
            self.history_all.append((self.counter, self.z))
        else:
    ...
    def single_step(self):
        ...
        for idx in idxs:
            self.queue.append(self.get_cross_data(idx, self.z[idx], z_proposal[idx]))
    ...
    def get_cross_data(self, idx, z_from, z_to):
        return idx, z_from, z_to

    def get_result(self):
        return self.counter, self.z, list(self.queue)

    def get_particle_log(self):
        return list(self.history_all)
...
class MH_MCMC_Hypersurface_ext(MH_MCMC_Hypersurface):
    ''' For extended data format
    '''
    def get_cross_data(self, idx, z_from, z_to):
        return self.counter, idx, z_from, z_to

    def get_result(self):
        return self.counter, self.z, list(self.queue), list(self.history_all)
...
def create_sampler(gp_r, origin, lb, ub, sigma=50, history=False):
    ...
    if history:
        class_sampler = MH_MCMC_Hypersurface_ext
    else:
        class_sampler = MH_MCMC_Hypersurface
    mcmc = class_sampler(move_big, likelihood_boundary, tester_inside, history=history)
    return mcmc

'''
class Sample_Hypersurface(object):
    def __init__(self, tester, num_init, ndim, origin, step_back, gp_r, gp_noise_var,
                 num_trajec=100):
        u_all = lhs_hypersphere(ndim, num_init)
        u_all, r_all, d_all, poff_all, detected_all, time_all, extra_meas_all = get_full_data(u_all, tester, step_back, origin=origin)
        ...
        self.tester = tester
        self.ndim, self.origin, self.step_back = ndim, origin, step_back
        self.gp_r, self.gp_noise_var = gp_r, gp_noise_var
        self.u_all, self.r_all, self.d_all = u_all, r_all, d_all
        self.poff_all, self.detected_all = poff_all, detected_all
        self.randomwalk = rw.Randomwalk_GPHypersurface(gp_r, origin, lb_short, ub_short)

    def __call__(self, num_samples, points_boundary=None, points_inside=100):
        points_boundary = [] if points_boundary is None
        if np.isscalar(points_inside):
            points_inside = random_points_inside(self.ndim, points_inside, self.gp_r, self.origin)

        for i in range(num_samples):
            if len(points_boundary) != 0:
                v = pick_from_boundary_points(points_boundary) 
                u = v / np.sqrt(np.sum(np.square(v-origin))) # voltage -> unit vector
            else:
                if i is not 0:
                    print('WARNING: no boundary point is sampled, a random direction will be used.')
                u = random_hypersphere(box_dim, 1)[0]

            #r, vols_pinchoff, found = self.tester.get_r(u, origin=self.origin) # Do measurement
            u, r, d, poff, detected, time, extra_meas= get_full_data(u[np.newaxis,:], self.tester, self.step_back, origin=self.origin)

            # Random movement while waiting the measurement


            # Project old samples to inside of the new estimated surface
            points_inside = project_points_to_inside(points_inside, gp_r, origin)
            points_inside, points_boundary = self.randomwalk(points_inside, 1000) # Sampling


            # Append the new data
            self.u_all = np.concatenate((self.u_all, u[np.newaxis,:]))
            self.r_all = np.append(self.r_all, r)

            # Update the estimated surface
            self.gp_r.create_model(self.u_all, self.r_all[:,np.newaxis], self.gp_noise_var, noise_prior='fixed')
            self.gp_r.optimize(num_restarts=20, opt_messages=False, print_result=True)
'''
...
def get_full_data(u_all, tester, step_back, origin=0.0, do_extra_meas=None, save_dir=None):
    ...
    for i, u in enumerate(u_all):
        ...
        d_vec, poff_vec, meas_each_axis, vols_each_axis = tester.measure_dist_all_axis(vols_pinchoff+step_back)
